...
static int main_thread(const char *conffile)
{
	...
	do
	{
		...
		if(usb_ctx)
		{
			...
			if( strstr(mtp_context->usb_cfg.usb_device_path, "mtp_usb") || mtp_context->usb_cfg.usb_functionfs_mode == 2 )
			{
				PRINT_MSG("uMTP Responder : Android Kernel mtp_usb Mode - running IO thread");
				usb_ctx->stop = 0;
				io_thread(usb_ctx);
			}
			else if( mtp_context->usb_cfg.usb_functionfs_mode )
			{
