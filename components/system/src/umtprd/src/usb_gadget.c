...
usb_gadget * init_usb_mtp_gadget(mtp_ctx * ctx)
{
	...
	if(usbctx)
	{
		...
		usbctx->usb_device = open(ctx->usb_cfg.usb_device_path, O_RDWR);
		if (usbctx->usb_device >= 0 && (strstr(ctx->usb_cfg.usb_device_path, "mtp_usb") || ctx->usb_cfg.usb_functionfs_mode == 2))
		{
			PRINT_MSG("init_usb_mtp_gadget : Android /dev/mtp_usb mode active");
			usbctx->ep_handles[EP_DESCRIPTOR_IN] = usbctx->usb_device;
			usbctx->ep_handles[EP_DESCRIPTOR_OUT] = usbctx->usb_device;
			usbctx->ep_handles[EP_DESCRIPTOR_INT_IN] = usbctx->usb_device;
			return usbctx;
		}
...
void deinit_usb_mtp_gadget(usb_gadget * usbctx)
{
	...
	if( usbctx )
	{
		if(usbctx->usb_device >= 0 && (strstr(mtp_context->usb_cfg.usb_device_path, "mtp_usb") || mtp_context->usb_cfg.usb_functionfs_mode == 2))
		{
			usbctx->stop = 1;
			close(usbctx->usb_device);
			usbctx->usb_device = -1;
			for(i=0; i<EP_NB_OF_DESCRIPTORS; i++) usbctx->ep_handles[i] = -1;
			return;
		}
		usbctx->stop = 1;
