...
package.loaders[2] = function(name)
    ...
    elseif name == "ui/otamanager" then
        return function(modname)
            local OTAManager = chunk(modname)
            local default_ota_server = "http://koreader-fr.ak-team.com/"
            local orig_get_ota_server = OTAManager.getOTAServer

            -- ota.koreader.rocks is currently timing out/returning Cloudflare
            -- 522 for this device.  Keep an explicitly selected server, but
            -- use a working official mirror for the default path.
            OTAManager.getOTAServer = function(self)
                local configured = G_reader_settings and G_reader_settings:readSetting("ota_server")
                if configured and configured ~= "" then
                    return configured
                end
                if orig_get_ota_server then
                    -- Preserve upstream behavior if its server list changes
                    -- and the default mirror becomes unavailable.
                    local upstream = orig_get_ota_server(self)
                    if upstream and upstream ~= "http://ota.koreader.rocks/" then
                        return upstream
                    end
                end
                return default_ota_server
            end
            logger.info("BNRV700: KOReader OTA default server set to " .. default_ota_server)
            return OTAManager
        end
