...
local function mxc_update(fb, ioc_cmd, ioc_data, is_flashing, waveform_mode, x, y, w, h, dither)
    ...
    -- On initial startup refresh, force a full GC16 hardware refresh to wake panel
    if marker <= 1 then
        ioc_data.update_mode = C.UPDATE_MODE_FULL
        ioc_data.waveform_mode = C.WAVEFORM_MODE_GC16
    end
    ...
    local actual_cmd = ioc_cmd
    local actual_data = ioc_data
    if fb.use_mxcfb_v1 then
        actual_cmd = C.MXCFB_SEND_UPDATE_V1
        if not fb.update_data_v1 then
            fb.update_data_v1 = ffi.new("struct mxcfb_update_data_v1")
        end
        fb.update_data_v1.update_region = ioc_data.update_region
        fb.update_data_v1.waveform_mode = ioc_data.waveform_mode
        fb.update_data_v1.update_mode = ioc_data.update_mode
        fb.update_data_v1.update_marker = marker
        fb.update_data_v1.temp = ioc_data.temp
        fb.update_data_v1.flags = ioc_data.flags
        actual_data = fb.update_data_v1
    end

    if C.ioctl(fb.fd, actual_cmd, actual_data) == -1 then
        local err = ffi.errno()
        fb.debug("MXCFB_SEND_UPDATE ioctl failed:", ffi.string(C.strerror(err)))
        if not fb.use_mxcfb_v1 and (ioc_cmd == C.MXCFB_SEND_UPDATE_V2 or ioc_cmd == C.MXCFB_SEND_UPDATE_V1_NTX) and (err == 25 or err == 22) then
            if not fb.update_data_v1 then
                fb.update_data_v1 = ffi.new("struct mxcfb_update_data_v1")
            end
            fb.update_data_v1.update_region = ioc_data.update_region
            ...
            fb.update_data_v1.temp = ioc_data.temp
            fb.update_data_v1.flags = ioc_data.flags
            if C.ioctl(fb.fd, C.MXCFB_SEND_UPDATE_V1, fb.update_data_v1) ~= -1 then
                fb.use_mxcfb_v1 = true
                fb.debug("MXCFB_SEND_UPDATE_V1 fallback succeeded, permanently enabled!")
            else
                fb.debug("MXCFB_SEND_UPDATE_V1 fallback failed:", ffi.string(C.strerror(ffi.errno())))
            end
        end
    end
