...
export WPA_SUPPLICANT_DRIVER="nl80211"
...
apply_bnrv700_shims() {
    if [ -d /opt/bnrv700-shims ]; then
        echo "==> [start_koreader] Syncing BNRV700 hardware shims into /opt/koreader..."
        cp -pf /opt/bnrv700-shims/*.sh /opt/koreader/ 2>/dev/null || true
        if [ -d /opt/bnrv700-shims/plugins ]; then
            mkdir -p /opt/koreader/plugins
            cp -rpf /opt/bnrv700-shims/plugins/* /opt/koreader/plugins/ 2>/dev/null || true
        fi
        chmod +x /opt/koreader/*.sh 2>/dev/null || true
    fi
    if [ -f /opt/1-bnrv700-hardware.lua ]; then
        mkdir -p /opt/koreader/patches
        cp -pf /opt/1-bnrv700-hardware.lua /opt/koreader/patches/1-bnrv700-hardware.lua 2>/dev/null || true
    fi
}
